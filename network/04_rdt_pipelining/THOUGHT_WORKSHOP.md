...
- **궁극의 통합(Synthesis):**
  - 현대 TCP는 GBN의 **[단일 타이머 + 누적 ACK의 간결성]**을 계승하되, GBN의 치명적 약점인 '순서가 맞지 않는 패킷(Out-of-order)의 가차 없는 폐기(Discard)'를 버리고, SR의 **[수신자 버퍼링]**을 취함으로써 **'정밀 재전송을 통한 네트워크 자원의 경제성'**을 달성한 모델이다. (즉, 유실된 패킷(Gap)을 송신자로부터 받은 순간, 입구 컷 당했던 버퍼 내의 패킷들까지 한꺼번에 상위 앱으로 Deliver하고, 가장 큰 연속된 Seq 번호 + 1을 ACK로 보냄으로써 송신자의 윈도우를 한 번에 점프시키는 하이브리드 전략이다.)
...
### 🎙️ 퀘스트 06: Selective Repeat & Flow Control - "정밀 정찰의 정수"

#### 🏺 1단계: 초기 발화 (Raw Extraction) [2026-04-05]

> "Selective Repeat(SR)은 송신자로부터 패킷을 받을 때 수신자 버퍼를 사용하여 순서가 뒤바뀌거나 유실되어도 일단 저장합니다. 이를 통해 송신자가 전체를 다시 보내는 오버헤드를 줄입니다. 또한 송신자는 각 패킷마다 타이머를 붙여서 타임아웃 시 해당 패킷만 재전송합니다. 수신자는 버퍼링을 통해 이빨 빠진 부분(Gap)이 메워지면 상위 앱으로 순서대로 올립니다. 학술적 딜레마(Dilemma)는 윈도우 크기가 시퀀스 번호의 절반을 넘으면 옛날 패킷과 새 패킷을 구분 못 하는 현상인데, 현대 TCP는 32비트 시퀀스 공간과 16비트 윈도우 크기를 사용하여 이를 구조적으로 해결했습니다."

#### 🛡️ 2단계: 전술적 교정 (Tactical Correction)

- **핵심:** GBN(연대 책임) vs SR(개별 책임)의 대조를 명확히 하고, **Re-ordering** 부하와 **대역폭 효율성**을 기술적 키워드로 배치함.
- **구조적 차이 3대 포인트:**
  1. **송신자:** 단일 타이머(GBN) vs 패킷별 독립 타이머(SR)
  2. **수신자:** 버퍼 없음(GBN) vs 수신자 버퍼 할당(SR)
  3. **ACK:** 누적 ACK(GBN) vs 개별 ACK(SR)

#### 🎙️ 3단계: 최종 각인 (Final Impression) [Rank S] 🏆

- **발화 핵심:** "넘버링 전술(첫째, 둘째, 셋째)을 사용하여 시니어 면접관에게 논리적 안정감을 전달함. 특히 0원도우 프로브(Zero-window probe) 시나리오를 통해 흐름 제어와 혼잡 제어의 상호작용을 설계자 수준에서 증명함."

---

### 🧬 [Deep-Dive] Zero-Window Probe 3대 전술 시나리오 분석

1. **🟢 CASE 1: 정상 복구 (Standard)**
   - `Probe` 성공 & `rwnd > 0` 회신 -> 송신자 윈도우 재개 및 파이프라이닝 속행.
2. **🟡 CASE 2: 정밀 복구 (Fast Retransmit/Recovery)**
   - `Probe` 전송 후 `Duplicate ACK` 회신 -> 수신자 버퍼는 비었으나 특정 Gap 유실 판단. `cwnd = cwnd / 2` (TCP Reno) 적용 및 빠른 재전송.
3. **🔴 CASE 3: 혼잡 붕괴 (Timeout/Tahoe)**
   - `Probe` 조차 유실(Timeout) -> 네트워크 전면 혼잡 판단. `cwnd = 1 MSS` (TCP Tahoe) 강제 감소 및 Slow Start 진입.

> **Insight:** 1바이트의 정찰병(Probe)은 수신자의 건강(Buffer)뿐만 아니라 네트워크 망의 건강(Congestion)까지 진단하는 송신자의 **지팡이**와 같다..
